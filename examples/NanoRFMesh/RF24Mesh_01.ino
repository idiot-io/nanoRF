...
void loop() {
  ...
  if (millis() - displayTimer >= 100) {
    displayTimer = millis();
    myData.A = map(analogRead(A0), 1023, 0, 0, 1023);
    myData.B = map(analogRead(A1), 1023, 0, 0, 1023);
    myData.C = map(analogRead(A2), 1023, 0, 0, 1023);
    myData.D = map(analogRead(A3), 1023, 0, 0, 1023);
